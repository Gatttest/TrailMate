...
.controller('MapCtrl', ['$scope', '$rootScope', '$state', '$window', 'leafletData', 'trailLocator', 'GeoJSONLayers', 'TrailCache', 'sharedUtils','$ionicSideMenuDelegate', '$http', '$rootScope', '$ionicLoading',
function($scope, $rootScope, $state, $window, leafletData, trailLocator, GeoJSONLayers, TrailCache, sharedUtils, $ionicSideMenuDelegate, $http,$rootScope, $ionicLoading) {
...
if($ionicSideMenuDelegate.isOpenLeft())
    ...
    url: 'https://{s}.tile.thunderforest.com/cycle/{z}/{x}/{y}.png?apikey=5e743e06f95d4c64a4bc68b7439ed75b'
        },
        defaults: {
   ...
   // Draw all the things
    ...
    // leafletData.getMap().then(function(map) {
    //         northEast = L.latLng(14.38,78.6211);
    //         southWest = L.latLng(14.2310, 78.7249);
...
//SHARE FUNCTION
 ...
 function onSuccess1(position) {
  ...
  subject: 'Oxfam Trailwalk 2018', // fi. for email
...
window.plugins.socialsharing.shareWithOptions(options2, onSuccess2, onError2);
...
.controller('loginCtrl', ['$scope', '$rootScope', '$window', '$http', 'sharedUtils','$ionicViewService', '$state', function($scope, $rootScope, $window, $http, sharedUtils, $ionicViewService, $state) {
    $scope.user = {};
...
console.log(window.device.platform);
       ...
       str="http://132.148.144.248/signin.php?t="+$scope.user.teamid+"&registrationId="+localStorage.getItem('registrationId')+"&m="+$scope.user.mobile+"&platform="+window.device.platform;
            $http.get(str)
...
// sendSmsApp.sendSms();
                     ...
                     $rootScope.km=window.localStorage.getItem('km');
...
.controller('weatherCtrl', function($scope, $window, $ionicLoading , $ionicViewService ,$http, $ionicSideMenuDelegate){
 ...
 var directions = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
  ...
  $http.get('http://api.openweathermap.org/data/2.5/weather?id=1277333&APPID=541e4e62dc286b79c6ccec9b171e4745').success(function (weather) {
...
.controller('helpCtrl', function($scope, $window, $ionicLoading, $ionicSideMenuDelegate, $ionicViewService){
...
.controller('POIDetailCtrl', function($scope, $stateParams, $state, $ionicViewService, trailLocator){
    ...
    $scope.goToPoi = function(poi){
        trailLocator.center = {
            lat: poi.geojson.coordinates[1],
            lng: poi.geojson.coordinates[0],
            zoom: 15
        };
        $state.go('tab.map');
    };
